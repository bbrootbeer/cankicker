...
void printCANMessage(const char* label, const CAN_message_t& msg);
...
const unsigned long errorCheckInterval = 500;
...
void loop() {
  while (can1.read(msg)) printCANMessage("CAN1", msg);
  while (can2.read(msg)) printCANMessage("CAN2", msg);

  if (millis() - lastErrorCheck > errorCheckInterval) {
    can1.error(err1, false);
    if (strcmp(err1.state, "Idle") != 0) printCANError("CAN1", err1);

    can2.error(err2, false);
    if (strcmp(err2.state, "Idle") != 0) printCANError("CAN2", err2);
